/*
=============================================================================
URBANO Col2 | Workflow 04b (variant: MAX) — National Integration
### Integración Urbana - Argentina ###
Colección: 3  |  Serie temporal: 1985-2025 (41 años)
Estrategia Pampa: MAX (valor máximo entre partes, excluyendo 27)
...
aplica .max() → si cualquier parte clasifica el píxel como urbano (24),
el resultado es 24 (el urbano prevalece sobre el 0).
...
Valores: 24 = urbano, 0 = no urbano, 27 = no observado (excluido del max)
=============================================================================
...
DESCRIPTION:
  ...
  ecoregion-boundary overlaps with an explicit priority table (with a few
  city-specific exceptions), and combining Pampa's 3 parts with `.max()`
  — a pixel is urban (24) if ANY of the 3 parts says so.

  See workflow/04b-integration-mosaic.js for the alternative Pampa-merge
  strategy (first-valid-value instead of max).
...
OUTPUT:
  - National multi-year binary classification, one `classification_<year>`
    band per year (1985-2025). Exported as `urbano-col2-prefilter-max`.
...
var output_name  = "urbano-col2-prefilter-max";
...
// Pampa: combina las 3 partes con max()
// Si alguna parte clasifica el píxel como 24 (urbano), el resultado es 24.
// Si todas tienen 27 (ya enmascarado), el píxel queda sin dato.
...
im_out = im_out
  ...
  .set('strategy',   'pampa-max')
  .set('years',      '1985-2025')
  .set('n_bands',    41)
  .set('description','Integración prefilter urbano - Argentina - 1985/2025 - Pampa: max (mask27)');
