/*
======================================================
### TF2 · SUAVIZADO — r1 (kernel=1)                ###
### Argentina | COLLECTION-3 | 1985-2025           ###
======================================================
Lee TF1 y aplica TF2 (suavizado). REMUEVE urbano.
  interm [1985-2022]: cur + 3 futuros, >=2/4
  penult [2023,2024]: -2,-1,cur,+1, >=2/4
  last   [2025]:      -2,-1,cur, >=1/3
Exporta: urban_TF2_r1_1985_2025_v2

PREVIOUS STEP: TF1.js
NEXT STEP:     TF3.js
*/

var config = {
  // 🔁 REPLACE: TF1 output.
  input_asset:  'projects/YOUR-PROJECT/assets/LAND-COVER/COLLECTION-3/GENERAL/URBAN/CLASSIFICATION/FILTERS/urban_TF1_r1_1985_2025_v2',
...
var exportAsset = function(binImg, name){
  var out = binImg.multiply(URBAN).toByte().set({
    ...
    'theme':'Urban Area','source':'MapBiomas Argentina','filter_type':'TF2',
...
var applyTF2 = function(m){
  return ee.Image.cat(YEARS.map(function(year){
    var cur = sel(m, year), pass;
    if (year <= END - 3) pass = cur.add(sel(m,year+1)).add(sel(m,year+2)).add(sel(m,year+3)).gte(2);
    else if (year <= END - 1) pass = sel(m,year-2).add(sel(m,year-1)).add(cur).add(sel(m,year+1)).gte(2);
    else pass = sel(m,year-2).add(sel(m,year-1)).add(cur).gte(1);
    return pass.multiply(cur).rename(bn(year));
  }));
};

var input = toBinary(ee.Image(config.input_asset));
var tf2 = applyTF2(input);
exportAsset(tf2, 'urban_TF2_r1_1985_2025_v2');
print('TF2 (suavizado) listo. Revisar Tasks.');
